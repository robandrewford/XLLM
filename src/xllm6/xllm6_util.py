"""Utility functions for XLLM6.

This module contains functions to read, create, or update embeddings and related tables
needed to retrieve data when processing a user query.
"""
...
def read_table(file_name, type="hash", path="", format="string"):
    """Read table from file."""
    table_local = {} if type == "hash" else []

    if path == "":
        # If file_name includes a path, use it directly
        file_name_with_path = file_name
    else:
        file_name_with_path = path + file_name

    try:
        file = open(file_name_with_path, "r", encoding="utf-8")
        lines = file.readlines()
        file.close()
    except (FileNotFoundError, IOError) as e:
        # If file not found locally, try GitHub
        print(f"Reading {file_name} from GitHub: {e}")
        url = "https://raw.githubusercontent.com/VincentGranville/Large-Language-Models/main/xllm6/"
        try:
            response = requests.get(url + file_name)
            content = response.text
            lines = content.split("\n")
            # Save a local copy
            file = open(file_name_with_path, "w", encoding="utf-8")
            file.write(content)
            file.close()
    ...
    # Process the lines based on type and format
    for line in lines:
        line = line.strip()
        if not line:
            continue
            ...
            if len(parts) >= 2:
                ...
                for item in parts[1:]:
                    if ":" in item:
                        k, v = item.split(":", 1)
                        if format == "float":
                            table_local[key][k] = float(v)
                        else:
                            table_local[key][k] = v
                    elif item:
                        if format == "float":
                            table_local[key][item] = 1.0
                        else:
                            table_local[key][item] = 1
        else:  # type == "list"
            table_local.append(line)

    return table_local


def read_arr_url(file_name, path=""):
    """Read array url from file."""
    arr_url_local = []
    ...
    try:
        ...
        try:
            ...
            print(f"Error downloading from GitHub: {e}")
            return arr_url_local

    for line in lines:
        line = line.split("\t")
        if len(line) > 1:
            arr_url_local.append(line[1])
    return arr_url_local


def read_stopwords(file_name, path=""):
    ...
    stopwords_local = content.split(",")
    return tuple(stopwords_local)


def read_dictionary(file_name, path=""):
    ...
    for line in lines:
        line = line.split("\t")
        if len(line) > 1:
            dictionary_local[line[0]] = int(line[1])
    return dictionary_local
...
def stem_data(data, stopwords, dictionary, mode="Internal"):
    ...
    for word in temp_dictionary:
        if mode == "Internal":
            n = len(word)
            if n > 2 and "~" not in word and word[0 : n - 1] in dictionary and word[n - 1] == "s":
